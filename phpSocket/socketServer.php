...
class socketServer
{
    ...
    const LISTEN_SOCKET_NUM = 1;
    const LOG_PATH = "./log/";  //日志
    private $_ip = "127.0.0.1"; //ip
    private $_port = 1888;  //端口 要和前端创建WebSocket连接时的端口号一致
    ...
    private function doEvents($socket, $recv_msg)
    {
        ...
        switch ($msg_type) {
            case 'login':
            // 登陆上线信息
                $this->_socketPool[(int)$socket]['userInfo'] = array("username" => $msg_content, 'page' => $recv_msg['page'], "login_time" => date("h:i"));
                // 取得最新的名字记录
